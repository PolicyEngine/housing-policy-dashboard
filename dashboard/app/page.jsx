...
const TAB_OPTIONS = [
  ...
  { id: "baseline", label: "Housing baseline" },
...
function Dashboard() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const response = await fetch("/data/rent_control_results.json");
        if (!response.ok) {
          throw new Error("rent_control_results.json not found");
        }
    ...
    <div className="app-shell min-h-screen">
      <header className="title-row">
        <div className="mx-auto flex max-w-[1400px] items-center justify-between px-6 py-4 md:px-8">
          <h1>UK rent control policy dashboard</h1>
        </div>
      </header>
      ...
      <main className="relative z-[1] mx-auto max-w-[1400px] px-6 py-10 md:px-8 md:py-12">
        <div className="animate-[fadeIn_0.4s_ease-out]">
          <p className="mb-3 text-[1.05rem] leading-relaxed text-slate-600">
            ...
            levels, tenure patterns, and benefit spending. The{" "}
            <strong>Methodology</strong> tab explains the model, data sources,
            and caveats.
          </p>
        </div>
